...
import Controller.EtatCreationRectangle;
import Controller.VueControlleurJeu;
import Controller.VueControlleurState;
...
public class FenetreGlobale extends JFrame implements ActionListener{
    ...
    public FenetreGlobale(VueControlleurJeu controlleurJeu, VueControlleurState etatCreationCercle){
        ...
        drawArea.add(new VueJeu2D((EtatCreationCercle) etatCreationCercle), BorderLayout.CENTER);
        frame.add(drawArea, BorderLayout.SOUTH);
         //JPanel vue = new VueGroupeFormesArea((EtatCreationCercle) etatCreationCercle);
         //frame.add(vue, BorderLayout.SOUTH);
         //vue.setPreferredSize(new Dimension(DIM, DIM-100)); // Donne une taille au JPanel
         //vue.setBackground(Color.WHITE); // Fond rouge pour drawArea
        ...
        //vue.addMouseListener(controlleurJeu);
        //vue.addMouseMotionListener(controlleurJeu); 
    ...
    public void actionPerformed(ActionEvent ae) {
        if(ae.getSource() == bouttonCercle){
            //EtatCreationCercle etat =  new EtatCreationCercle();
            //etatCreationCercle = etat;
            controlleurJeu.setEtatCourant(etatCreationCercle);
            //controlleurJeu.setEtatCreationCercle(etatCreationCercle);      
            System.out.println("cercleeeee");
        }
        if(ae.getSource() == bouttonRectangle){
            controlleurJeu.setEtatCourant(new EtatCreationRectangle());
            System.out.println("rectangleeee");
        }
    }
}
